...
import pymysql
from ui_py.create_nominas_quincena import Ui_Form
...
class createNominasQuincenal(QWidget, Ui_Form):
    def __init__(self):   
       super().__init__()
       self.setupUi(self)
    ...
    def vals(self):
        ...
        try:
            with connection.cursor() as cursor:
                ...
                FROM employee e
                LEFT JOIN nominas n ON e.cedula = n.cedula
                LEFT JOIN (
...
"""
    ...
    def clicked_row(self, index: QModelIndex):
            ...
            self.nominaInfo.open_window(cedula, 'Q')
    ...
    def create_nomina(self):
        ...
        query = "SELECT e.cedula FROM employee_prev e WHERE e.nomina_type = 'Quincenal';"
        cursor.execute(query)
        #self.vals()
        ...
        for empleado in empleados:
             ...
             payment = payment[0]
             payment = payment
             asignaciones = asignaciones_user + payment + conf.prima_hogar/2 + conf.prima_transporte/2 + conf.prima_alimentacion/2 + (conf.prima_hijo*float(hijos[0]))/2 + (conf.prima_profesion[profession]/100)*(payment) + (conf.prima_anti[min(str(service_years), '23')]/100)*(payment)
             ...
             if self.half:
                 selected_date_start = current_date.replace(day=16).strftime('%d-%m-%Y')
                 ...
                 selected_date_end = current_date.replace(day=15).strftime('%d-%m-%Y')
             ...
             self.create_nomina_fnt(cedula_user, 
                                                   payment,
                                                   round(conf.prima_transporte/2,2),
                                                   round(conf.prima_hogar/2,2),
                                                   round(conf.prima_alimentacion/2,2),
                                                   round(conf.prima_hijo*float(hijos[0])/2, 2),
