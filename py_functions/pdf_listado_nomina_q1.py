...
def generate_pdf_lista(tipo_nomina):
    try:
        ...
        if tipo_nomina == "S":
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    """
                    SELECT 
                        nomina_semanal_prev.cedula, employee.tipo_cuenta, employee.nro_cuenta, 
                        employee.name, employee.lastname, nomina_semanal_prev.deducciones_T, nomina_semanal_prev.total_neto, employee.nomina_type
                    FROM 
                        nomina_semanal_prev
                    JOIN 
                        employee ON nomina_semanal_prev.cedula = employee.cedula
                """
                )
                nominas = cursor.fetchall()

            # Cálculo de totales
            nomina_type = nominas[0][7]
            ...
            if file_path:
                ...
                img_path_alcaldia = os.path.join(
                    base_dir, "../images/alcaldia_logo5.png"
                )
                ...
                hora_actual = hora_24.strftime("%I:%M").lstrip(
                    "0"
                )  # Remover el 0 inicial si existe
                periodo = (
                    "a.m." if hora_24.hour < 12 else "p.m."
                ...
                for index, nomina in enumerate(nominas):
                    ...
                    if index == 0 or contador % 25 == 0:
                        ...
                        c.drawImage(
                            img_path_alcaldia, 300, 750, alcaldia_width, img_height
                        )

                        # Encabezados de las celdas
                        cuadro_y = (
                            660 if index == 0 else 730
                        )  # Cambiar posición dependiendo de la página
                ...
                QMessageBox.warning(
                    None,
                    "Cancelado",
                    "No se seleccionó ubicación para guardar el archivo.",
                )
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    """
                    SELECT 
                        nominas.cedula, employee.tipo_cuenta, employee.nro_cuenta, 
                        employee.name, employee.lastname, nominas.deducciones_T, nominas.total_neto, employee.nomina_type
                    FROM 
                        nominas
                    JOIN 
                        employee ON nominas.cedula = employee.cedula
                """
                )
                nominas = cursor.fetchall()
        ...
        QMessageBox.critical(
            None, "Error", f"Error al conectarse a la base de datos: {str(e)}"
        )
